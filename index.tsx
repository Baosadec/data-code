...
const fetchChartData = async (timeFrame: TimeFrame): Promise<ChartDataPoint[]> => {
  ...
  return btcKlines.map((k: any) => {
    ...
    return { time: timeLabel, timestamp: timestamp, btc: btcClose, xau: xauClose };
  });
};
...
const runDeepLearningAnalysis = async (marketData: any) => {
  ...
  try {
    ...
    return "⚠️ Neural Network Connection Failed. Check API Key or Quota.";
  }
};
  ...
  const renderControls = () => (
    <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
      <div className="flex bg-slate-900/50 p-1 rounded-lg border border-slate-700/50">
        <button onClick={() => onChartModeChange('combined')} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${chartMode === 'combined' ? 'bg-slate-700 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}>
          <Layers size={14} /> Overlay
        </button>
        <button onClick={() => onChartModeChange('btc')} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${chartMode === 'btc' ? 'bg-[#4ecdc4]/20 text-[#4ecdc4] border border-[#4ecdc4]/20' : 'text-slate-400 hover:text-slate-200'}`}>
          <DollarSign size={14} /> BTC Only
        </button>
        <button onClick={() => onChartModeChange('gold')} className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${chartMode === 'gold' ? 'bg-[#ffd700]/20 text-[#ffd700] border border-[#ffd700]/20' : 'text-slate-400 hover:text-slate-200'}`}>
          <Maximize2 size={14} /> Gold Only
        </button>
      </div>
      <div className="flex gap-1 bg-slate-900/50 p-1 rounded-lg border border-slate-700/50">
        {[ { label: '1H', value: TimeFrame.H1 }, { label: '4H', value: TimeFrame.H4 }, { label: '24H', value: TimeFrame.D1 }, { label: '7D', value: TimeFrame.D7 } ].map((tf) => (
          <button key={tf.value} onClick={() => onTimeFrameChange(tf.value)} className={`px-3 py-1 rounded text-xs font-bold transition-all ${timeFrame === tf.value ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`}>
            {tf.label}
          </button>
        ))}
      </div>
      ...
      <div className="h-[400px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            ...
            <XAxis dataKey="time" stroke="#64748b" tick={{ fill: '#64748b', fontSize: 10, fontWeight: 500 }} tickLine={false} axisLine={false} minTickGap={40} dy={10} />
            <YAxis yAxisId="left" stroke={leftAxisColor} tick={{ fill: leftAxisColor, fontSize: 11, fontFamily: 'monospace' }} domain={['auto', 'auto']} tickFormatter={(val) => chartMode === 'btc' || chartMode === 'combined' ? `$${(val/1000).toFixed(1)}k` : `$${val}`} axisLine={false} tickLine={false} width={50} />
            {showRightAxis && (
              <YAxis yAxisId="right" orientation="right" stroke="#ffd700" tick={{ fill: '#ffd700', fontSize: 11, fontFamily: 'monospace' }} domain={['auto', 'auto']} tickFormatter={(val) => `$${val}`} axisLine={false} tickLine={false} width={50} />
            )}
            ...
            {(chartMode === 'combined' || chartMode === 'btc') && (
              <Area yAxisId="left" type="monotone" dataKey="btc" name="Bitcoin (BTC)" stroke="#4ecdc4" strokeWidth={2} fill="url(#gradientBtc)" activeDot={{ r: 6, fill: '#1e1e2e', stroke: '#4ecdc4', strokeWidth: 2 }} animationDuration={800} />
            )}
            {(chartMode === 'combined' || chartMode === 'gold') && (
              <Area yAxisId={chartMode === 'combined' ? "right" : "left"} type="monotone" dataKey="xau" name="Gold (XAU)" stroke="#ffd700" strokeWidth={2} fill="url(#gradientXau)" activeDot={{ r: 6, fill: '#1e1e2e', stroke: '#ffd700', strokeWidth: 2 }} animationDuration={800} />
            )}
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    ...
    <div className="bg-slate-900 rounded-xl border border-indigo-500/30 p-1 relative overflow-hidden shadow-2xl group">
      ...
      <div className="bg-slate-950/80 backdrop-blur-md rounded-lg p-5 h-full relative z-10 flex flex-col">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-5 gap-4 border-b border-indigo-500/20 pb-4">
          ...
          <div className="flex items-center gap-2">
            {!process.env.API_KEY && (
               <div className="text-xs text-red-400 flex items-center gap-1 mr-2 bg-red-900/20 px-2 py-1 rounded border border-red-500/30">
                 <ShieldAlert size={12} /> Config Error: API Key Missing
               </div>
            )}
            <button
